...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    add_two_array.push(arr[i] + 2);
  }
  return add_two_array;
  // Solution code here...
};
...
const typeNum = (arr) => {
  const type_num_arr = arr.filter((element) => typeof element === 'number');
  return type_num_arr;
  // Solution code here...
};
...
const containsAnd = (arr) => {
  const contains_and_array = arr.filter((string) => string.includes('and'));
  return contains_and_array;
  // Solution code here...
};
...
const oddValues = (arr) => {
  const odd_values_array = arr.filter((odds) => odds % 2 === 1);
