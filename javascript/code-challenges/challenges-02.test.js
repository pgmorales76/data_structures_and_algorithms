...
const addOne = (arr) => {
  const incremented_array = arr.map((number) => number + 1);
...
const addQuestion = (arr) => {
  const add_question_array = arr.map((string) => `${string}?`);
  return add_question_array;
  // Solution code here...
};
...
const forLoopTwoToThe = (arr) => {
  const loop_two_array = [];
  for (let i = 0; i < arr.length; i++) {
    loop_two_array.push(Math.pow(2, arr[i]));
  }
  return loop_two_array;
  // Solution code here...
};
